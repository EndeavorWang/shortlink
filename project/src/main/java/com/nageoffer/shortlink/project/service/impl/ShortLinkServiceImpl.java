...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        ...
        // 保存短连接和长连接到 redis中，加快跳转速度，因为要从短连接服务器获取长连接
        ...
        // 完整短连接添加到布隆过滤器, 感觉这个没必要吧，只需放入唯一标识就行了，因为域名是一致的
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        ShortLinkDO shortLinkDO = ShortLinkDO.builder()
                ...
                .validDate(requestParam.getValidDate())
                .build();
        // 更改的组是否与原分组相同，相同，则更新，不同，则先删除再插入
    ...
    // 生成域名后面的后缀 http://domain/suffix
    private String generateSuffix(ShortLinkCreateReqDTO requestParam) {
        // 已经为长链接生成的短链接数量
        int customGenerateCount = 0;
        // todo 判断长链接是否已经生成过短连接
        if (shortUriCreateCachePenetrationBloomFilter.contains(requestParam.getOriginUrl())) {
            throw new ServiceException("该链接的短链接已经存在，无法重复生成");
        }
        ...
        while (true) {
            ...
            // 布隆过过滤器查重，直到查到一个唯一的url为止
            if (!shortUriCreateCachePenetrationBloomFilter.contains(shorUri)) {
                break;
            }
            // 这里使用的domain为什么是前端传过来的domain
            /*if (!shortUriCreateCachePenetrationBloomFilter.contains(requestParam.getDomain() + "/" + shorUri)) {
                break;
            }*/
            customGenerateCount++;
        }
        shortUriCreateCachePenetrationBloomFilter.add(shorUri);
        // todo 添加长连接到布隆过滤器中，说明已经生成过短连接了
        shortUriCreateCachePenetrationBloomFilter.add(requestParam.getOriginUrl());
        return shorUri;
    }
